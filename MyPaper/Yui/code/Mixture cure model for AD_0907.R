...
AD.1<-read.csv("C:/github_LTY/Lee_Tsung_Yu/MyPaper/Yui/AD_data.csv")
head(AD.1)
str(AD.1)
table(AD.1$event)
table(AD.1$APOE)
...
mean(AD.1[AD.1$Gender==0,9]);sqrt(var(AD.1[AD.1$Gender==0,9]));mean(AD.1[AD.1$Gender==1,9]);sqrt(var(AD.1[AD.1$Gender==1,9]))
median(AD.1[AD.1$Gender==1,9])

mean(AD.1[AD.1$APOE==0,9]);sqrt(var(AD.1[AD.1$APOE==0,9]));mean(AD.1[AD.1$APOE==1,9]);sqrt(var(AD.1[AD.1$APOE==1,9]))
median(AD.1[AD.1$APOE==0,9])

wilcox.test(AD.1[AD.1$Gender==0,9],AD.1[AD.1$Gender==1,9])
wilcox.test(AD.1[AD.1$APOE==0,9],AD.1[AD.1$APOE==1,9])

cor.test(AD.1$CASI_score,AD.1$CDR_score)
...
#para.test.0<-c(gamma.00,beta.00)
gamma.storage<-c()
beta.storage<-c()
tryCatch({
while(1){
...
Non.Congi<-cbind(AD.2[,1],AD.2[,3],non.cong)
hist(non.cong,main="The probability of never experiencing dementia until death")
...
(log(2)/exp(beta.t.01[1]+(beta.t.01[2]*1)+(beta.t.01[3]*1)+(beta.t.01[4]*84)))^(1/alpha.t.S1)
