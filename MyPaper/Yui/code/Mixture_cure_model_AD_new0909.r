...
# table(AD.1$Event) # 0: 86(28.6%), 1: 215(71.4%)
# table(AD.1$APOE) # 0: 176(58.5%), 1: 125(41.5%)
...
sWei <- survreg(s ~ Gender+Education+APOE+CDR_score,dist='weibull',data=AD.2)
summary(sWei)
(predict(sWei, newdata=list(Gender=0,Education=9,APOE=0,CDR_score=0.5),type="quantile",p=0.5))

S <- glm(event~Gender+APOE+CASI_score,data=AD.2,family = binomial(link = "logit"))
summary(S)
