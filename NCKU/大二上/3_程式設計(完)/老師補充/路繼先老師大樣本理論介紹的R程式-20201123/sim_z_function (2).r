...
simu.z <- function(dstn="norm", n=31, mu, sigma, ..., R=1000, plot.it=TRUE) { # ... : 可以放其他的東西，後面的...可以之後再定義
  ### Simulation study of z-statistic, namely, CLT
  dist <- sapply(c("d", "r"), function(k) get(paste(k, dstn, sep=""))) 
  #t/s/l apply :代替回圈 : 把＂Ｄ＂帶進去算一遍，再把＂Ｒ＂帶進去算一遍
  if(missing(mu))
...
simu.z(dstn="norm", n=31, mu=0, sigma=1, R=1000, plot.it=TRUE)
